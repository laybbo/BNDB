...
def list_top_gainers(
    ...
    *,
    ...
    app_config = config or AppConfig()
    http = session or build_session()
    response = http.get(f"{app_config.binance_base_url}/fapi/v1/ticker/24hr", timeout=30)
    response.raise_for_status()
    payload = response.json()
    ordered = sorted(
        (
        ...
        key=lambda item: float(item.get("priceChangePercent", 0.0)),
        reverse=True,
    )
    return [
        {
