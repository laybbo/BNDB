...
from bndb.definitions import price_shock_signal
from bndb.detectors.base import BaseDetector


class PriceShockDetector(BaseDetector):
    name = "price_shock"
    description = "Detect sharp 5m price expansion with volatility expansion."
    ...
    def detect(
        ...
        for index in range(48, len(klines)):
            ...
            triggered, score, detail = price_shock_signal(current, history, self.thresholds)
            if triggered:
                events.append(
                    {
                        "symbol": symbol,
