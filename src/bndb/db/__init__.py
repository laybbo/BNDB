...
def upsert_watchlist_entries(database_path: str | Path, rows: list[dict[str, str]]) -> int:
    if not rows:
        return 0
    with connect(database_path) as connection:
        before = connection.total_changes
...
def list_watchlist(database_path: str | Path) -> list[dict[str, str]]:
    with connect(database_path) as connection:
        rows = connection.execute(
            "SELECT symbol, category, added_at, source FROM watchlist ORDER BY category, symbol"
        ).fetchall()
    return [dict(row) for row in rows]


def get_watchlist_markers(database_path: str | Path) -> dict[str, str]:
    return {row["symbol"]: row["category"] for row in list_watchlist(database_path)}
...
def count_events_by_type(database_path: str | Path) -> dict[str, int]:
    ...
    for row in rows:
        counter[row["event_type"]] = row["count"]
    return dict(counter)
...
def count_outcomes_by_path(database_path: str | Path) -> dict[str, int]:
    ...
    with connect(database_path) as connection:
        rows = connection.execute(
            "SELECT path_type, COUNT(*) AS count FROM event_outcomes GROUP BY path_type ORDER BY path_type"
        ).fetchall()
    for row in rows:
        counter[row["path_type"]] = row["count"]
    return dict(counter)
...
def fetch_event_report(database_path: str | Path, limit: int) -> list[dict[str, Any]]:
    with connect(database_path) as connection:
        rows = connection.execute(
            """
            SELECT e.symbol, e.event_type, e.score, e.triggered_at, o.path_type, w.category AS watchlist_category
            FROM events e
